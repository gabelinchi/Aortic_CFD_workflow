...
#get XML file
tree = ET.parse(r"C:/Users/pjjon/Documents/BEP/funderingen/Funderin2.0.feb")
root = tree.getroot()
...
def Parent4(pppparent, ppparent,pparent, parent, variable, value):
    ...
    return
#single parent
analysis_edit = Parent3('Step', 'step', 'Control', 'analysis', str(analysis))
time_steps_edit = Parent3('Step', 'step', 'Control', 'time_steps', str(time_steps))
step_size_edit = Parent3('Step', 'step', 'Control', 'step_size', str(step_size))
plot_zero_state_edit = Parent3('Step', 'step', 'Control', 'plot_zero_state', str(plot_zero_state))
plot_range_str = ','.join(str(x) for x in plot_range)
plot_range_edit = Parent3('Step', 'step', 'Control', 'plot_range', plot_range_str)
plot_level_edit = Parent3('Step', 'step', 'Control', 'plot_level', str(plot_level))
output_level_edit = Parent3('Step', 'step', 'Control', 'output_level', str(output_level))
plot_stride_edit = Parent3('Step', 'step', 'Control', 'plot_stride', str(plot_stride))
output_stride_edit = Parent3('Step', 'step', 'Control', 'output_stride', str(output_stride))
adaptor_re_solve_edit = Parent3('Step', 'step', 'Control', 'adaptor_re_solve', str(adaptor_re_solve))


#double parent
max_retries_edit = Parent4('Step', 'step','Control', 'time_stepper', 'max_retries', str(max_retries))
opt_iter_edit = Parent4('Step', 'step','Control', 'time_stepper', 'opt_iter', str(opt_iter))
dtmin_edit = Parent4('Step', 'step','Control', 'time_stepper', 'dtmin', str(dtmin))
aggressiveness_edit = Parent4('Step', 'step','Control', 'time_stepper', 'aggressiveness', str(aggressiveness))
cutback_edit = Parent4('Step', 'step','Control', 'time_stepper', 'cutback', str(cutback))
dtforce_edit = Parent4('Step', 'step','Control', 'time_stepper', 'dtforce', str(dtforce))
density_edit = Parent2('Material', 'material', 'density', str(density))
k_edit = Parent2('Material', 'material', 'k', str(k))

symmetric_stiffness_edit = Parent4('Step', 'step','Control','solver', 'symmetric_stiffness', str(symmetric_stiffness))
equation_scheme = Parent4('Step', 'step','Control', 'solver','equation_scheme', str(equation_scheme))
equation_order_edit = Parent4('Step', 'step','Control','solver', 'equation_order', str(equation_order))
optimize_bw_edit = Parent4('Step', 'step','Control','solver', 'optimize_bw', str(optimize_bw))
lstol_edit = Parent4('Step', 'step','Control', 'solver','lstol', str(lstol))
lsmin_edit = Parent4('Step', 'step','Control', 'solver','lsmin', str(lsmin))
lsiter_edit = Parent4('Step', 'step','Control','solver', 'lsiter', str(lsiter))
max_refs_edit = Parent4('Step', 'step','Control','solver', 'max_refs', str(max_refs))
check_zero_diagonal_edit = Parent4('Step', 'step','Control','solver', 'check_zero_diagonal', str(check_zero_diagonal))
zero_diagonal_tol_edit = Parent4('Step', 'step','Control','solver', 'zero_diagonal_tol', str(zero_diagonal_tol))
force_partition_edit = Parent4('Step', 'step','Control','solver', 'force_partition', str(force_partition))
reform_each_time_step_edit = Parent4('Step', 'step','Control','solver', 'reform_each_time_step', str(reform_each_time_step))
reform_augment_edit = Parent4('Step', 'step','Control','solver', 'reform_augment', str(reform_augment))
diverge_reform_edit  = Parent4('Step', 'step','Control','solver', 'diverge_reform', str(diverge_reform))
min_residual_edit = Parent4('Step', 'step','Control','solver', 'min_residual', str(min_residual))
max_residual_edit = Parent4('Step', 'step','Control','solver', 'max_residual', str(max_residual))
vtol_edit = Parent4('Step', 'step','Control','solver', 'vtol', str(vtol))
ftol_edit = Parent4('Step', 'step','Control','solver', 'ftol', str(ftol))
rhoi_edit = Parent4('Step', 'step','Control','solver', 'rhoi', str(rhoi))
etol_edit = Parent4('Step', 'step','Control','solver', 'etol', str(etol))
rtol_edit = Parent4('Step', 'step','Control','solver', 'rtol', str(rtol))
predictor_edit = Parent4('Step', 'step','Control','solver', 'predictor', str(predictor))
min_volume_ratio_edit = Parent4('Step', 'step','Control','solver', 'min_volume_ratio', str(min_volume_ratio))

zerofluidvelocity_x_edit = Parent4('Step', 'step','Boundary', 'bc', 'wx_dof', str(int(zerofluidvelocity_x)))
zerofluidvelocity_y_edit = Parent4('Step', 'step','Boundary', 'bc', 'wy_dof', str(int(zerofluidvelocity_y)))
zerofluidvelocity_z_edit = Parent4('Step', 'step','Boundary', 'bc', 'wz_dof', str(int(zerofluidvelocity_z)))

velocity_edit = Parent2('Loads', 'surface_load', 'velocity', str(velocity))
nodal_velocities_edit = Parent2('Loads', 'surface_load', 'prescribe_nodal_velocities', str(int(prescribe_nodal_velocities)))
parabolic_edit  = Parent2('Loads', 'surface_load', 'parabolic', str(int(parabolic)))
prescribe_rim_pressure_edit  = Parent2('Loads', 'surface_load', 'parabolic', str(int(prescribe_rim_pressure)))

#triple parent
kappa_edit = Parent3('Material', 'material', 'viscous', 'kappa', str(kappa))
mu_edit = Parent3('Material', 'material', 'viscous', 'mu', str(mu))
...
viscous.set('type',str(viscoustype))
 """
#create FEBio file
tree.write("bouwwerk.feb",encoding='ISO-8859-1', xml_declaration=True,)

#Run FEBio
FEBio_path = r"C:/Program Files/FEBioStudio2/bin/febio4.exe"
FEBio_args = r"C:/Users/pjjon/Documents/BEP/Git/Aortic_CFD_workflow/bouwwerk.feb"
FEBio_inputfile = r"C:/Users/pjjon/Documents/BEP/Git/Aortic_CFD_workflow/pijpie.feb"
subprocess.run([FEBio_path, FEBio_args], check = True)

#it now created a bouwwerk.xplt file which contains the results