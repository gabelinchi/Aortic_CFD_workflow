function [F_new,V_new,F_new_wall,V_new_wall] = cutVessel_bifurcation_wall(F_fs,V_fs,F_wall,V_wall,F_inlet,V_inlet,F_outlet1,V_outlet1,F_outlet2,V_outlet2) 

[F_new,V_new,F_new_wall,V_new_wall] = Subtraction(F_fs,V_fs,F_wall,V_wall,F_inlet,V_inlet,1,1);
[F_new,V_new,F_new_wall,V_new_wall] = Subtraction(F_new,V_new,F_new_wall,V_new_wall,F_outlet1,V_outlet1,1,2);
[F_new,V_new,F_new_wall,V_new_wall] = Subtraction(F_new,V_new,F_new_wall,V_new_wall,F_outlet2,V_outlet2,2,2);
...
function [F_out,V_out,F_out_wall,V_out_wall] = Subtraction(F_in,V_in,F_in_wall,V_in_wall,F_ref,V_ref,typex,typez) %type = 1 or 2, depending on that the bias for the cylinder is set tu -bias (inverted) - this quantity is used for translation of the cylinder used for boolean operation (in function subtraction)
...
biasx = 2;
biasz = 2;
r_cylinder = 3.5*r_ref; %il cilindro deve avere raggio 3 volte piu grande del massimo raggio della sezione
...
[F_box,V_box] = CreateCylinder(r_cylinder,V_ref,typez); %created cylinder for subtraction -> another function
...
gpatch(F_in_wall,V_in_wall,'kw','none',0.3)
view(45,25)
axis equal
%close(gcf);
...
[F_new_wall,V_new_wall] = booleanOperation(F_in_wall,V_in_wall,F_box,V_box,1);

[G1] = tesgroup(F_new); %assign index 1 or 2 to the faces F_new depending on which side they belong to (after the cutting, we have 2 parts/sides)
[G2] = tesgroup(F_new_wall);
...
%Process to get rid of the extremes generated after the boolean operation with the cylinder
 num = sum(G1);
 [~,ind_save] = max(num);
 F_new = F_new(G1(:,ind_save),:);

  num = sum(G2);
 [~,ind_save] = max(num);
 F_new_wall = F_new_wall(G2(:,ind_save),:);

%Process to get rid of the inlet/outlet surfaces 
a=(30/180)*pi;
G1=patchFeatureDetect(F_new,V_new,a); %Detect surface features
clear num
for i = 1:max(G1)
    num(i) = sum(ismember(G1,i));
end
num_sort = sort(num,'descend');
F_test = F_new(G1==find(num==num_sort(2)),:);
figure;
gpatch(F_new,V_new,'kw','none',0.3);
gpatch(F_test,V_new);
view(45,25)
axis equal
F_out = F_new(G1~=find(num==num_sort(2)),:);
V_out = V_new;
[~,~] = patchCleanUnused(F_out,V_out);
close(gcf)

a=(30/180)*pi;
G2=patchFeatureDetect(F_new_wall,V_new_wall,a); %Detect surface features
clear num
for i = 1:max(G2)
    num(i) = sum(ismember(G2,i));
end
num_sort = sort(num,'descend');
F_test = F_new_wall(G2==find(num==num_sort(2)),:);
figure;
gpatch(F_new_wall,V_new_wall,'kw','none',0.3);
gpatch(F_test,V_new_wall);
view(45,25)
axis equal
F_out_wall = F_new_wall(G2~=find(num==num_sort(2)),:);
V_out_wall = V_new_wall;
[~,~] = patchCleanUnused(F_out_wall,V_out_wall);
close(gcf)
 
figure('WindowState', 'maximized');
 gpatch(F_out,V_out);
 gpatch(F_in,V_in,'kw','none',0.3)
 gpatch(F_out_wall,V_out_wall,'kw','none',0.3)
 title('Result of cutting operation', 'num', 'fontSize',25);
 view(45,25)
 axis equal
 %close(gcf)
...
function [F_box,V_box] = CreateCylinder(r_cylinder,V_ref,biasz)
...
OR = V_mean / norm(V_mean)*(norm(V_mean)+biasz);
T  = [1 0 0 OR(1);...
      0 1 0 OR(2);...
      0 0 1 OR(3);...
      0 0 0 1];
